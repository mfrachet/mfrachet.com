...
Some people can stand that it's probably not necessary, and I agree. The thing is you have it for free. It's baked in Remix and works using the same API no matter you submit the form from JavaScript or from the HTML form itself.
...
Here comes the limitation of not being able to modify the internals of Astro directly.
...
- if we submit the form without JavaScript, the `action` function will be executed and its returned value will be passed to the React context. Then Remix will re-render the whole page and send it back as a document with the updated content.
- if we submit the form with JavaScript, the `action` function is called internally using `fetch`. Remix has now access to the data and can pass it down via context. It only needs to re-render on the client the content is updated.

In our situation, we can't do that. We can't go higher than the route itself and we don't have access to the same rendering mechanism as React (also, islands might get us in trouble).

We now have multiple solution, they are less elegant than the Remix one (which is basically "magic"). I'm going with the one on top of my head which is adding `data-*` attribute and update every of them if they exist.

The HTML code would now look like:

```html
<main>
  <h1>Welcome</h1>
  <!--  Since we return `data` now, we need to update from `error` to `data?.error` -->
...
And we then need to adjust the `handleClientForm` function to update these node when we got an answer from the backend:
...
With this code, the UI will update after the fetch request is made. Of course, this
implementation is one way to achieve this. You can probably use other APIs or approaches
to achieve this.
