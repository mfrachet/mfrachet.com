...
Another note: the code for the feature flag evaluation can be done on the client side (in an SDK for example) or the server side (directly on your service API) depending on the tradeoffs you're ok with.
...
We now have a unique identifier (that is very probably a string) that is consistent across sessions that we can use to make computations. But how can we compare a `string` value to a percentage range?
